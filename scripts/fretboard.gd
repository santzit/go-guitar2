...
const UPCOMING_MARKER_Z: float = 0.055
...
func update_upcoming_markers(events: Array, song_time: float, lookahead: float, start_idx: int = 0) -> void:
	if _upcoming_root == null:
		return
	for c in _upcoming_root.get_children():
		c.free()
...
func _render_event_markers(notes: Array) -> void:
	...
	for n in notes:
		...
		if s < 0 or s >= STRING_COUNT:
			continue
		if f == 0:
			open_notes.append({"fret": f, "string": s})
	...
	if fretted.size() == 1:
		var sn: Dictionary = fretted[0]
		_add_single_marker(int(sn.get("fret", 1)), int(sn.get("string", 0)))
	elif fretted.size() > 1:
...
func _add_single_marker(fret: int, string_idx: int) -> void:
	...
	marker.set_surface_override_material(0, _make_marker_material(Color(1.0, 1.0, 1.0, 1.0), 0.7))
...
func _add_open_string_marker(string_idx: int) -> void:
	...
	marker.set_surface_override_material(0, _make_marker_material(Color(1.0, 1.0, 1.0, 1.0), 0.55))
	_upcoming_root.add_child(marker)
...
func _add_chord_group_marker(notes: Array) -> void:
	...
	marker.set_surface_override_material(0, _make_marker_material(Color(0.8, 0.95, 1.0, 1.0), 0.65))
	_upcoming_root.add_child(marker)


func _make_marker_material(color: Color, emission_energy: float) -> StandardMaterial3D:
