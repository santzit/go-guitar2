...
func _ready() -> void:
	...
	if ClassDB.class_exists("PitchDetector"):
		...
		if _pitch_detector.start(sample_rate):
			if _input_audio_manager != null and not _input_audio_manager.samples_ready.is_connected(_on_guitar_samples):
...
func _on_guitar_samples(player_id: int, pcm_bytes: PackedByteArray) -> void:
	...
	var target_idx: int = _selected_string_idx if _selected_string_idx >= 0 else _find_closest_target_string(freq_hz)
...
func _on_string_button_pressed(string_idx: int) -> void:
	...
	_update_selected_string_label()
	if _has_detection:
		_apply_live_ui(_selected_string_idx if _selected_string_idx >= 0 else _find_closest_target_string(_smoothed_freq_hz))
...
func _freq_to_note_name(freq_hz: float) -> String:
	if freq_hz <= 0.0:
		return "--"
	var midi: int = int(round(69.0 + 12.0 * (log(freq_hz / 440.0) / log(2.0))))
...
func _note_to_freq(note: String) -> float:
	if note.length() < 2:
		return 0.0
	var octave: int = int(note.substr(note.length() - 1, 1))
	var name: String = note.substr(0, note.length() - 1)
