...
/// The SPSC ring buffers and DSP thread for Godot block I/O are **not** in this
/// file.  They live in the C++ bridge (`q_dsp_bridge.cpp`) and are managed by
/// `RtEngine` in `rt_engine.rs`.  `EngineCore` handles only the CPAL audio path.
///
/// ## Lifecycle
/// ```rust
